...
def signup():
    if request.method=="POST":
        ...
        user = User.query.with_entities(User.id, User.srfid, User.email, User.password).filter_by(srfid=srfid).first()
        emailUser=User.query.filter_by(email=email).first()
        if user or emailUser:
            flash("Email or srif is already taken","warning")
            return render_template("usersignup.html")
       
        new_user=User(srfid=srfid,email=email,password=generate_password_hash(
...
def addhospitalinfo():
    ...
    if request.method=="POST":
        ...
        if huser:            
            query=Hospitaldata(hcode=hcode,hname=hname,normalbed=nbed,icubed=ibed,vbed=vbed)
            db.session.add(query)
            db.session.commit()
...
def hedit(id):
    ...
    if request.method=="POST":
        ...
        vbed=request.form.get('ventbeds')
        hcode=hcode.upper()
        # db.engine.execute(f"UPDATE `hospitaldata` SET `hcode` ='{hcode}',`hname`='{hname}',`normalbed`='{nbed}',`icubed`='{ibed}',`vbed`='{vbed}' WHERE `hospitaldata`.`id`={id}")
    ...
    if request.method == "POST":
        ...
        if bedtype == "NormalBed":
            if dbb.normalbed > 0:
                ...
                flash("No available normal beds", "danger")
                return render_template("booking.html", query=query, query1=query1)
            ...
            if dbb.icubed > 0:
                ...
                flash("No available ICU beds", "danger")
                return render_template("booking.html", query=query, query1=query1)
            ...
            if dbb.vbed > 0:
                ...
                flash("No available ventilator beds", "danger")
                return render_template("booking.html", query=query, query1=query1)
